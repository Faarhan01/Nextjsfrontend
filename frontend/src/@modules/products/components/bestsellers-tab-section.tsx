...
export const BestsellersTabSection: React.FC<BestsellersTabSectionProps> = ({
  ...
  // Filter products according to active tab with safe fallback
  const filteredProducts = React.useMemo(() => {
    let result = products.filter((p) => {
      if (activeTab === 'sale') return p.isSale || Boolean(p.originalPrice);
      if (activeTab === 'new') return p.isFeatured || (p as any).isNew || (p as any).stockStatus === 'lowstock';
      if (activeTab === 'rated') {
        const rating = (p as any).rating || 0;
        const reviewsCount = (p as any).reviewsCount || (p as any).reviews?.length || 0;
        return rating >= 4 || reviewsCount > 0 || p.isFeatured;
      }
  ...
  return (
    <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-6 border-b border-slate-200 dark:border-slate-800">
        <div>
          <span className={clx('text-xs font-extrabold uppercase tracking-widest', currentTheme.text)}>
            Curated Collections
          </span>
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={clx(
                  'px-3.5 py-2 rounded-xl text-xs font-extrabold transition-all duration-200 flex items-center gap-1.5 cursor-pointer shrink-0',
                  isActive
                    ? clx(currentTheme.bg, 'text-white shadow-md shadow-blue-500/15')
                    : 'bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200'
                )}
              >
                <Icon className={clx('w-3 h-3', isActive ? 'text-white' : 'text-slate-500 dark:text-slate-400')} />
                <span>{tab.label}</span>
              </button>
            );
          })}
            ...
            >
              ...
              >
                ...
                <div className="absolute top-2.5 right-2.5 z-10 flex flex-col gap-1.5">
                  ...
                  >
                    <Heart className={clx('w-4 h-4 stroke-[2.5]', isWishlisted && 'fill-rose-500 text-rose-500')} />
                  </button>
                  {onQuickView && (
                    <button
                      onClick={(e) => {
              ...
              <div className="p-3 sm:p-3.5 flex-1 flex flex-col justify-between bg-white dark:bg-slate-800">
                <div className="cursor-pointer" onClick={() => onSelectProduct(prod.id)}>
                  ...
                  <h3 className={clx('text-xs sm:text-sm font-extrabold text-slate-900 dark:text-white transition-colors line-clamp-2 leading-snug w-full', currentTheme.groupHoverText)}>
                    {prod.name}
                  </h3>
                ...
                <div className="pt-2.5 mt-2.5 border-t border-slate-100 dark:border-slate-700/80 flex items-center justify-between gap-2">
                  ...
                  {onAddToCart && (
                    <button
                      ...
                      className={clx('p-2 rounded-xl text-white hover:opacity-90 active:scale-95 transition-all shadow-xs flex items-center justify-center shrink-0 cursor-pointer', currentTheme.bg)}
                      title="Add to Cart"
                    >
      ...
      <div className="text-center pt-8">
        <button
          ...
          className={clx('inline-flex items-center gap-2 px-6 py-3 rounded-2xl text-white font-bold text-xs sm:text-sm shadow-md transition hover:scale-102 active:scale-98 cursor-pointer', currentTheme.bg)}
        >
