...
export const CartButton: React.FC<CartButtonProps> = ({ isActive = false }) => {
  ...
  return (
    <button
      ...
      className={clx(
        'w-8.5 h-8.5 sm:w-9 sm:h-9 flex items-center justify-center rounded-full border transition-all duration-150 cursor-pointer relative hover:scale-105 active:scale-95 shrink-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/40',
        cartOpen || isActive
          ? clx(currentTheme.lightBg, currentTheme.border.split(' ')[0])
