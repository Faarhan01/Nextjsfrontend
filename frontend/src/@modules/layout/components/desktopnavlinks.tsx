  ...
  return (
    <nav className="hidden lg:flex items-center space-x-1 lg:space-x-1.5 text-xs sm:text-sm font-semibold flex-shrink-0">
      ...
      <div className="relative group">
        ...
        >
          <span>Company</span>
          <ChevronDown className="w-3 h-3 transition-transform duration-200 group-hover:rotate-180 text-slate-400 dark:text-slate-500 group-hover:text-slate-700 dark:group-hover:text-slate-300" />
        </button>
      ...
      {(currentUser?.role === 'admin' || isActive('/admin')) && (
        ...
        >
          <ShieldCheck className={`w-3 h-3 ${currentTheme.text} dark:text-blue-400`} />
          <span>Admin</span>
        </Link>
      )}
    </nav>
