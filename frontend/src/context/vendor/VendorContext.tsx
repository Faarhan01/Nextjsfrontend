...
export function VendorProvider({ children }: { children: ReactNode }) {
  ...
  const handleCreateOrUpdateOffer = useCallback(
    (productId: string, offer: Partial<VendorOffer> & { price: number; stockCount: number; condition: ProductCondition }) => {
      setSellerAccounts((prev) =>
        prev.map((seller) => ({
          ...seller,
          products: seller.products.map((product) =>
            product.id === productId
              ? {
                  ...product,
                  offers: product.offers.map((existingOffer) =>
                    existingOffer.id === offer.id ? { ...existingOffer, ...offer } : existingOffer
                  ),
                }
              : product
          ),
  ...
  const handleDeleteOffer = useCallback((productId: string, offerId: string) => {
    setSellerAccounts((prev) =>
      prev.map((seller) => ({
        ...seller,
        products: seller.products.map((product) =>
