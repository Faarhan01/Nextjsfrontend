...
import { Star, CheckCircle, ThumbsUp, MessageSquare, Send, Loader2, Filter } from 'lucide-react';
import { payload } from '../../services/payloadClient';
import { PayloadReviewDoc } from '../../types/payload';
import { DEFAULT_STORE_REVIEWS } from '../../utils/productRating';

interface PayloadReviewsProps {
  productId: string;
  productTitle?: string;
  themeColor?: string;
}

export function PayloadReviews({ productId, productTitle = 'Product', themeColor = 'blue' }: PayloadReviewsProps) {
  // Compute initial fallback reviews instantly so the UI never blocks with a long loader
  const initialLocalReviews: PayloadReviewDoc[] = React.useMemo(() => {
  ...
  const [reviews, setReviews] = useState<PayloadReviewDoc[]>(initialLocalReviews);
  const [totalReviews, setTotalReviews] = useState<number>(initialLocalReviews.length);
  const [averageRating, setAverageRating] = useState<number>(() => {
    if (initialLocalReviews.length === 0) return 5.0;
    const sum = initialLocalReviews.reduce((acc, r) => acc + r.rating, 0);
    return sum / initialLocalReviews.length;
  });
  const [ratingDist, setRatingDist] = useState<Record<number, number>>(() => {
    const dist: Record<number, number> = { 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 };
    initialLocalReviews.forEach(r => {
  ...
  const fetchReviews = async () => {
    try {
      const res = await payload.reviews.find(productId);
      if (res && res.docs && res.docs.length > 0) {
        setReviews(res.docs);
        setTotalReviews(res.totalDocs);
        setAverageRating(res.averageRating || 5.0);
        if (res.ratingDistribution) {
          setRatingDist(res.ratingDistribution);
        }
      }
    } catch (err) {
      console.warn('Failed to load reviews from Payload CMS:', err);
    }
  };

  useEffect(() => {
    if (productId) {
      setReviews(initialLocalReviews);
      fetchReviews();
    }
  }, [productId, initialLocalReviews]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await payload.reviews.create({
        ...
        comment: formComment,
        authorName: formAuthor || 'Verified Patron'
      });
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-6 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
            ...
            {[5, 4, 3, 2, 1].map(stars => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 text-xs group text-left ${selectedFilter === stars ? 'font-bold text-blue-600 dark:text-blue-400' : 'text-slate-600 dark:text-slate-400'}`}
                >
        ...
        <div>
          <button
            ...
            className="w-full md:w-auto px-5 py-2.5 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold text-xs hover:bg-slate-800 dark:hover:bg-slate-100 transition-all flex items-center justify-center gap-2 shadow-sm"
          >
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="p-6 rounded-2xl border border-blue-200 dark:border-blue-900/50 bg-blue-50/40 dark:bg-blue-950/20 space-y-4 animate-in fade-in">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 font-medium">
              Payload CMS 3.88 Verified Buyer
            </span>
          </div>
          ...
          {submitSuccess ? (
            <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 text-xs font-semibold flex items-center gap-2">
              ...
              <span>Thank you! Your verified review has been published to Payload CMS.</span>
            </div>
          ) : (
            <button
              type="submit"
              disabled={isSubmitting || !formComment.trim()}
              className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs disabled:opacity-50 transition-all flex items-center gap-2 shadow-sm"
            >
      ...
      {isLoading ? (
        <div className="py-12 text-center text-slate-400 space-y-2">
          ...
          <p className="text-xs">Fetching verified reviews from Payload CMS...</p>
        </div>
