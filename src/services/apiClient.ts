...
class ApiClient {
  ...
  async login(email: string, password?: string): Promise<{ success: boolean; user: UserProfile; token: string }> {
    try {
      ...
      const user: UserProfile = {
        ...
        email: cleanEmail,
        phone: '+27 (0) 11 837 2000',
        shippingAddress: {
          addressLine1: '150 Industrial Rd, Crown North',
          city: 'Johannesburg',
          province: 'Gauteng',
          postalCode: '2092',
      ...
      this.setToken(token);
      if (typeof window !== 'undefined') {
        localStorage.setItem('luxestore_user_profile', JSON.stringify(user));
      }
  ...
  async getProfile(): Promise<{ success: boolean; user: UserProfile }> {
    try {
      return await this.request<{ success: boolean; user: UserProfile }>('/auth/me');
    } catch {
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('luxestore_user_profile');
        if (stored) {
          try {
  ...
  logout() {
    this.setToken(null);
    if (typeof window !== 'undefined') {
      localStorage.removeItem('luxestore_user_profile');
    }
  }
  ...
  async placeOrder(orderData: {
    ...
    try {
      const res = await this.request<{ success: boolean; order: any; message: string }>('/orders', {
        method: 'POST',
        body: JSON.stringify(orderData)
      });
      // Also cache in localStorage for instant tracking
      if (typeof window !== 'undefined' && res?.order) {
        try {
          const existing = JSON.parse(localStorage.getItem('luxestore_orders') || '[]');
          existing.unshift(res.order);
          localStorage.setItem('luxestore_orders', JSON.stringify(existing));
        } catch {}
      }
      return res;
    } catch {
  ...
  async trackOrder(orderId: string): Promise<{ success: boolean; order: any }> {
    try {
      return await this.request<{ success: boolean; order: any }>(`/orders/track/${encodeURIComponent(orderId)}`);
    } catch {
      if (typeof window !== 'undefined') {
        try {
          const existing = JSON.parse(localStorage.getItem('luxestore_orders') || '[]');
          const cleanId = orderId.trim().toUpperCase();
          const match = existing.find((o: any) => o.orderNumber?.toUpperCase() === cleanId || o.id?.toUpperCase() === cleanId);
          if (match) {
      ...
      return {
        ...
        order: {
          ...
          items: [{ name: 'Mrbulk Catalog Order Item', quantity: 1, price: 'Confirmed' }],
        },
      };
    }
  }
  ...
  async getUserOrders(email?: string): Promise<{ success: boolean; count: number; orders: any[] }> {
    try {
      ...
      return await this.request<{ success: boolean; count: number; orders: any[] }>(`/orders/user${qs}`);
    } catch {
      let localOrders: any[] = [];
      if (typeof window !== 'undefined') {
        try {
          localOrders = JSON.parse(localStorage.getItem('luxestore_orders') || '[]');
        } catch {}
      }
